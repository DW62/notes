...
来源不同：sleep() 来自 Thread，wait() 来自 Object。
对锁的影响不同：sleep() 不释放锁；wait() 释放锁。
恢复方法不同：sleep() 时间到会自动恢复；wait() 可以使用 notify()/notifyAll()直接唤醒。
...
![image-20211218141423258](https://gitee.com/dwOrigin/imgck/raw/master/image/image-20211218141423258.png)

1、  用户发送请求至前端控制器DispatcherServlet。

2、 DispatcherServlet收到请求调用HandlerMapping处理器映射器。

3、 处理器映射器根据XML配置或者注解找到具体的处理器，生成处理器对象及处理器拦截器(如果有则生成)一并返回给DispatcherServlet。
